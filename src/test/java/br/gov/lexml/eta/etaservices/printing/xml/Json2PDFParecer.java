...
import br.gov.lexml.eta.etaservices.printing.pdf.TesteGeracaoPDF2;
...
class Json2PDFParecer {
    ...
    private static void process(String nomeArquivo) throws Exception {
        ...
        System.out.println("--------------------------------------------------------------");
        System.out.println("--------------------------------------------------------------");
        System.out.println(anexos.toString());
        pdfGenerator.generate(parecer, anexos, fos);
    }
    ...
    private static List<AnexoPDFA> criarAnexos() throws IOException {
        ...
        anexos.add(AnexoPDFA.builder().nomeArquivo("A.pdf").file(cl.getResourceAsStream("anexoA.pdf")).imprimir(true).build());

        anexos.add(AnexoPDFA.builder().nomeArquivo("B.pdf").file(cl.getResourceAsStream("anexoB.pdf")).imprimir(true).build());

        anexos.add(AnexoPDFA.builder().nomeArquivo("B.docx").file(cl.getResourceAsStream("anexoB.docx")).imprimir(false).build());
